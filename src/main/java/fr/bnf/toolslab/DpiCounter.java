...
/**
 * Counter to classify the different values. In order to group the values, only
 * multiples of 10 are kept, using the nearest value.
 *
 */
public class DpiCounter {
	...
	public Entry<Integer, Integer> getBest() {
		final Comparator<Map.Entry<Integer, Integer>> valueComparator = Map.Entry
	...
	public String toString() {
		final Comparator<Map.Entry<Integer, Integer>> keyComparator = Map.Entry
				.comparingByKey(Comparator.naturalOrder());
		return dpis.entrySet().stream().sorted(keyComparator)
				.map(e -> "DPI(" + e.getKey() + ")=" + e.getValue())
