 ...
 */
public class TiffColorExtractor extends TiffExtractor {
  protected static final Logger LOGGER = Logger.getLogger(TiffColorExtractor.class.getName());

  protected int idxBitsPerSample = 0;
   ...
   */
  public TiffColorExtractor(PDImageXObject image, int dpiX, int dpiY) {
  ...
  public boolean extract(File outputFile) {
    ...
    int numOfComponents = cspace.getNumberOfComponents();


    try (FileOutputStream fos = new FileOutputStream(outputFile)) {
      // Append IFDs to the header
      // cf https://www.awaresystems.be/imaging/tiff/specification/TIFF6.pdf
      addShort(calculateNbFields((short) 12)); // 2-byte count of the number of directory entries
      ...
      addField(SAMPLESPERPIXEL, TIFF_TYPE_SHORT, 1, numOfComponents); // 3
      ...
      // DATA
      setField(idxBitsPerSample, getPosition());
      for (int i = 0; i < numOfComponents; i++) {
        addShort((short) bitsPerComponent);
      }
      addResolutionData();
      addExtraData();
